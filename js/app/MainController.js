...
Controller.$inject = [
	...
	function Controller(mapService, loggerService, placesService, $timeout, $scope) {
		...
		$scope.infoPanel					= true;			//show/hide panel with town info
		...
		$scope.initApp	= function(_baseHref,_urlWMS,_urlWMSqgis,_environment,_token,_isMobile,_mapid,_mapname){
            ...
            // search equipaments
            $("#searchEquipTipus").on("change", function() {
            	//console.log(this.options[this.selectedIndex].value);
                var value = this.options[this.selectedIndex].value;

                mapService.setVisibleLayer("Equipaments");

            	$("#searchEquip").empty();
            	$("#searchEquip").append('<option value="-1">-  Triï una opció  -</option>');
                
                if (value !== -1) {
					log("getEquipament: "+value);

					placesService.getEquipament(value).then(function(data) {
					    // add results to select
					    for (var i in data.message) {
					    	var geom = JSON.parse(data.message[i].geom);
					    	$("#searchEquip").append('<option data-x="'+geom.coordinates[0][0]+'" data-y="'+geom.coordinates[0][1]+'">'+data.message[i].nom+'</option>');
					    }
					})
					.catch(function (error) {
					 	log("error in getEquipament:", error);
				    });
				}
            });

            $("#searchEquip").on("change", function() {
				var x = Number(this.options[this.selectedIndex].getAttribute('data-x'));
                var y = Number(this.options[this.selectedIndex].getAttribute('data-y'));
