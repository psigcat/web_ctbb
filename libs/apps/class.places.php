...
class Places {
	...
	******************************/
	...
	public function getStreetNum($id){
		$query		= "SELECT portal,ST_AsGeoJSON(geom) as geom from carrerer.car_portals where cod_via=".$id." order by length(portal), portal";
	...
	public function getCatasterRefFromPoligon($id){
		$query		= "SELECT masa,parcela,refcat,coorx,coory,ST_AsGeoJSON(ST_Transform(geom, 3857)) as geom FROM cadastre.parcela where masa='".$id."' AND tipo='U'";
